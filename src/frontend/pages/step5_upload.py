...
def render():
    ...
    # ===== FILE UPLOAD =====
    st.subheader("⬆️ อัพโหลดไฟล์วิดีโอ")
    
    st.info("💡 อัพโหลดไฟล์วิดีโอที่สร้างจาก Veo 3 (หรือไฟล์อื่นๆ) มาเก็บไว้ในโปรเจคนี้")
    ...
    # ===== UPLOADED FILES LIST =====
    st.subheader("📋 รายการไฟล์ในโปรเจค")
    ...
    if project.uploaded_files:
        for i, file_path in enumerate(project.uploaded_files):
            ...
            with col_delete:
                if st.button("ลบ", key=f"del_file_{i}"):
                    if file_exists:
                        os.remove(file_path)
        ...
        st.markdown("---")
        st.metric("จำนวนไฟล์ทั้งหมด", len(project.uploaded_files))
    else:
        st.info("ยังไม่มีไฟล์อัพโหลด")
    ...
    drive_link = st.text_input(
        "ลิงก์ Google Drive Folder (สำหรับส่งงาน)",
    ...
    if project.scenes and any(s.veo_prompt for s in project.scenes):
        st.subheader("✂️ Export Timeline สำหรับตัดต่อ")
        st.caption("นำออกไฟล์ Timeline เพื่อนำไปตัดต่อในโปรแกรม (ต้องวางไฟล์วิดีโอเอง)")
        ...
        try:
            ...
            with col_edl:
                ...
                st.download_button(
                    ...
                    help="สำหรับ Adobe Premiere Pro, DaVinci Resolve",
                )
            ...
            with col_otio:
                ...
                st.download_button(
                    ...
                    use_container_width=True,
                    help="OpenTimelineIO รูปแบบมาตรฐานกลาง",
                )
            ...
            with col_bundle:
                ...
                st.download_button(
                    ...
                    use_container_width=True,
                    help="ดาวน์โหลดไฟล์ทั้งหมดพร้อมคู่มือ",
                )

            st.success(f"✅ พร้อม Export Timeline ({len(project.scenes)} ฉาก / {project.total_duration:.1f}s)")

        except Exception as e:
            st.error(f"❌ Export ผิดพลาด: {e}")
    ...
    with col_save:
        if st.button("💾 บันทึกงาน", use_container_width=True):
    ...
    with col_complete:
        if st.button("🎉 จบโปรเจค", type="primary", use_container_width=True):
            ...
            st.balloons()
            st.success("ยินดีด้วย! โปรเจคเสร็จสมบูรณ์แล้ว!")
    ...
    if project.status == "completed":
        ...
        with col1:
            st.metric("จำนวนฉาก", len(project.scenes) if project.scenes else 0)
        
        with col2:
            st.metric("ไฟล์ที่ได้", len(project.uploaded_files) if project.uploaded_files else 0)
        
        with col3:
            duration = sum(s.audio_duration for s in project.scenes) if project.scenes else 0
            st.metric("ความยาวรวม", f"{duration:.1f}s")
